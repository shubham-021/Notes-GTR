...
When a file operation fails and there is no reasonable way to recover, use `panic` to immediately stop execution. This is common in scripts or initialization code where continuing without the file would be meaningless.

```go
f, err := os.Open("example.txt")
if err != nil {
    panic(err)  // Program stops here with error message
}
defer f.Close()
```

`panic` differs from returning an error:
- `panic` aborts the program immediately (unless recovered)
- Returning an error allows the caller to decide how to handle the failure

Use `panic` sparingly. In libraries and production code, prefer returning errors. Reserve `panic` for truly unrecoverable situations or quick scripts where you want fail-fast behavior.

---

## Getting File Information

Use `file.Stat()` or `os.Stat()` to retrieve metadata about a file without reading its contents.

### Using Stat on an Open File
...
fmt.Println("Is Directory:", fileInfo.IsDir())   // false
fmt.Println("File Size:", fileInfo.Size())       // size in bytes
fmt.Println("Permissions:", fileInfo.Mode())     // -rw-r--r--
fmt.Println("Last Modified:", fileInfo.ModTime()) // timestamp
```
...
| `Mode()` | `FileMode` | File mode and permission bits |
| `ModTime()` | `time.Time` | Last modification time |
| `IsDir()` | `bool` | Whether the file is a directory |

---

## Reading File Content into Bytes

For more control over reading, you can read file contents into a byte buffer manually.
...
// Create a buffer sized to the file
...
### Simpler Alternative with os.ReadFile

For most cases, `os.ReadFile` is cleaner:
